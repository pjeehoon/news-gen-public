...
class MultiArticleDeepAnalyzer:
        ...
        try:
            ...
            try:
                ...
                if videos:
                    ...
                    # 검색은 수행되었지만 결과가 없는 경우
                    logger.info(f"YouTube 검색 완료: '{search_query}'에 대한 결과 없음")
                    return []
            except ImportError:
            ...
            # 검색 모듈이 없거나 오류 발생 시 빈 리스트 반환
            logger.info("YouTube 검색을 수행할 수 없음 - 검색 건너뜀")
            return []
    ...
    def search_google_articles(self, keywords: list, num_results: int = 10) -> list:
        ...
        try:
            ...
            try:
                ...
                sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
                from external_search import search_google, search_wikipedia
    ...
    def search_wikipedia_background(self, keywords: list, num_results: int = 3) -> list:
        """Wikipedia에서 배경 정보 검색"""
        try:
            # 상위 키워드로 Wikipedia 검색
            wiki_results = []
            ...
            try:
                ...
                import os
                sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
                from external_search import search_wikipedia
                ...
                for kw in keywords[:3]:
                    if kw and len(kw) > 1:  # 너무 짧은 키워드 제외
                        search_keywords.append(kw)
                
                # 키워드 조합 추가 (더 구체적인 검색을 위해)
                if len(keywords) >= 2:
                    # 첫 두 키워드의 조합
                    combined = f"{keywords[0]} {keywords[1]}"
                    search_keywords.insert(0, combined)  # 우선순위 높임
                
                # 각 키워드별로 검색
                seen_titles = set()  # 중복 제거용
                
                for keyword in search_keywords[:4]:  # 최대 4개 검색어
                    if not keyword:
                        continue
                        
                    logger.info(f"Wikipedia 검색: {keyword}")
                    
                    # 한국어 Wikipedia 검색
                    ko_results = search_wikipedia(keyword, num_results=3, lang='ko')
                    ...
                    for result in ko_results:
                        ...
                        # 관련성 낮은 결과 필터링
                        # 선거, 정치인 등 일반적인 페이지는 제외
                        irrelevant_keywords = ['국회의원 선거', '대통령 선거', '지방선거']
                        if any(irr in title for irr in irrelevant_keywords):
                            # 키워드와 직접 관련이 없으면 스킵
                            if not any(kw in title for kw in keywords[:2]):
                                continue
                ...
                logger.info(f"Wikipedia에서 {len(wiki_results)}개 배경 정보 검색 완료")
                return wiki_results
                
            except ImportError:
                logger.warning("external_search 모듈을 찾을 수 없습니다")
            except Exception as e:
                logger.warning(f"Wikipedia API 검색 실패: {e}")
...
Wikipedia 배경: {json.dumps(analysis.get('wikipedia_context', []), ensure_ascii=False, indent=2)}
...
1. **정보 검증 가능성 평가**:
   ...
   - **반드시 related_articles의 url 필드에 있는 실제 URL을 사용할 것**
   - **Wikipedia 정보 활용**: 배경 설명이나 개념 정의시 Wikipedia 출처 명시
     * Wikipedia는 wikipedia_context에 있는 실제 url을 사용
     * 주제와 관련이 없는 Wikipedia 링크는 사용하지 말 것
...
[Wikipedia 등에서 제공하는 관련 주제의 객관적 배경 정보와 역사적 맥락]
[주의: Wikipedia 정보는 기사 주제와 직접적으로 관련된 경우에만 사용]
[관련 없는 선거나 정치인 페이지는 인용하지 말 것]
...
**외부 자료 활용 지침**:
- YouTube: 관련 영상의 핵심 내용을 기사에 자연스럽게 통합
- Google: 추가적인 배경 정보나 다른 관점 제시에 활용
- Wikipedia: 개념 설명, 역사적 배경, 객관적 사실 확인에 활용
  * 특히 "배경 정보" 섹션에서 Wikipedia 정보를 적극 활용
  * 예: "[위키백과](URL)에 따르면, [개념/사건]은..."
- 모든 외부 자료는 출처를 명확히 밝히고 링크 포함
...
**출처 링크 사용 원칙**:
- related_articles에 있는 각 기사의 url 필드 값을 그대로 사용
- YouTube 동영상은 youtube_context의 url 또는 link 필드 사용
- Wikipedia는 wikipedia_context의 url 필드 사용 (단, 주제와 관련 있는 경우만)
- 단순히 "(네이버뉴스)" 같은 텍스트가 아닌 "[네이버뉴스](실제URL)" 형식으로 작성
...
"""
    ...
    def analyze_topic(self, main_news_url: str, main_title: str, news_category: str = None, 
        ...
        # 4. Wikipedia 배경 정보 검색
        logger.info("\n4단계: Wikipedia 배경 정보 검색")
        wikipedia_results = self.search_wikipedia_background(keywords, num_results=3)
        if wikipedia_results:
            logger.info(f"Wikipedia에서 {len(wikipedia_results)}개 배경 정보 발견")
            cluster_result["wikipedia_background"] = wikipedia_results
        ...
        # YouTube, Google, Wikipedia 정보를 analysis에 추가 (프롬프트 크기 최적화)
        ...
        if "wikipedia_background" in cluster_result:
            analysis["wikipedia_context"] = cluster_result["wikipedia_background"][:4]  # 최대 4개
        ...
        # YouTube/Google/Wikipedia 정보 추가
        if "youtube_videos" in cluster_result:
        ...
        if "wikipedia_background" in cluster_result:
            result["wikipedia_background"] = cluster_result["wikipedia_background"]
            result["wikipedia_count"] = len(cluster_result["wikipedia_background"])
