...
def collect_article_metadata():
    ...
    for article_id, info in topic_index.items():
        ...
        article_meta = {
            ...
            'total_tokens': 0,
            'cost_usd': 0,
            'prompt_cost_usd': 0,  # 프롬프트 비용
            'completion_cost_usd': 0,  # 완성 비용
            'image_cost_usd': 0,  # 이미지 생성 비용
                ...
                if metadata_dir.exists():
                    ...
                    if best_match:
                        with open(best_match, 'r', encoding='utf-8') as f:
                            ...
                            # 개별 비용 계산 (GPT-4.1-nano 기준)
                            article_meta['prompt_cost_usd'] = (article_meta['prompt_tokens'] / 1000) * 0.0001
                            article_meta['completion_cost_usd'] = (article_meta['completion_tokens'] / 1000) * 0.0004
                            
                            # 이미지 생성 비용 확인
                            if 'image_generations' in token_metadata:
                                image_cost = sum(img.get('cost_usd', 0) for img in token_metadata['image_generations'])
                                article_meta['image_cost_usd'] = image_cost
                                # 총 비용에 이미지 비용 추가
                                article_meta['cost_usd'] = article_meta['cost_usd'] + image_cost
        ...
        # 이미지 메타데이터 찾기 (article_id로 직접 검색)
        image_metadata_file = Path(f"article_metadata/{article_id}_metadata.json")
        if image_metadata_file.exists():
            try:
                with open(image_metadata_file, 'r', encoding='utf-8') as f:
                    image_data = json.load(f)
                    if image_data.get('type') == 'image_generation' or 'image_generations' in image_data:
                        # 이미지 생성 비용 추가
                        if 'image_generations' in image_data:
                            article_meta['image_cost_usd'] = sum(img.get('cost_usd', 0) for img in image_data['image_generations'])
                        else:
                            article_meta['image_cost_usd'] = image_data.get('cost_usd', 0)
                        # 총 비용에 이미지 비용 추가
                        article_meta['cost_usd'] = article_meta['cost_usd'] + article_meta['image_cost_usd']
            except Exception as e:
                print(f"Error loading image metadata for {article_id}: {e}")
...
def calculate_statistics(metadata):
    ...
    daily_stats = defaultdict(lambda: {
        ...
        'completion_tokens': 0,
        'total_cost': 0,
        'prompt_cost': 0,      # 프롬프트 비용
        'completion_cost': 0,  # 완성 비용
        'image_cost': 0        # 이미지 비용
    })
    ...
    for article in metadata:
        ...
        if date:
            ...
            daily_stats[date]['prompt_cost'] += article.get('prompt_cost_usd', 0)
            daily_stats[date]['completion_cost'] += article.get('completion_cost_usd', 0)
            daily_stats[date]['image_cost'] += article.get('image_cost_usd', 0)
